...
from lmtk.text import QUOTES, normalize, latex_to_unicode, u, s
...
class PersonName(dict):
    ...
    def __init__(self, name=None, from_bibtex=False):
        """Initialize with a name string.

        :param name: A person name as a string.
    ...
    def __repr__(self):
        return '%s(%r)' % (self.__class__.__name__, s(self.name))
    ...
    def could_be(self, other):
        ...
        if self == other:
            return True
        for attr in [u'title', u'firstname', u'middlename', u'nickname', u'prefix', u'lastname', u'suffix']:
            ...
            if s == o:
                continue
            if attr in {u'firstname', u'middlename', u'lastname'}:
    ...
    def __getattr__(self, name):
        if name in {u'title', u'firstname', u'middlename', u'nickname', u'prefix', u'lastname', u'suffix'}:
    ...
    def _clean(self, t, capitalize=None):
        ...
        if self._from_bibtex:
            ...
            t = normalize(t, collapse=False)
        t = u' '.join([el.rstrip('.') if el.count('.') == 1 else el for el in t.split()])
        return t
    ...
    def _parse(self, name):
        ...
        if len(comps) > 1 and not all([self._is_suffix(comp) for comp in comps[1:]]):
            ...
            ltokens = self._strip(ltokens, self._is_prefix, u'prefix')
            ltokens = self._strip(ltokens, self._is_suffix, u'suffix', True)
            self[u'lastname'] = self._clean(u' '.join(ltokens), capitalize='name')
        tokens = self._tokenize(comps)
        tokens = self._strip(tokens, self._is_title, u'title')
        if not u'lastname' in self:
            tokens = self._strip(tokens, self._is_suffix, u'suffix', True)
        voni = []
        end = len(tokens) - 1
        if not u'prefix' in self:
            for i, token in enumerate(reversed(tokens)):
                if self._is_prefix(token):
                    if (i == 0 and end > 0) or (not u'lastname' in self and not i == end):
                        voni.append(end - i)
                else:
                    if (i == 0 and u'lastname' in self) or voni:
                        break
        if voni:
            if not u'lastname' in self:
                self[u'lastname'] = self._clean(u' '.join(tokens[voni[0]+1:]), capitalize='name')
            self[u'prefix'] = self._clean(u' '.join(tokens[voni[-1]:voni[0]+1]))
            tokens = tokens[:voni[-1]]
        else:
            if not u'lastname' in self:
                self[u'lastname'] = self._clean(tokens.pop(), capitalize='name')
        if tokens:
            self[u'firstname'] = self._clean(tokens.pop(0), capitalize='name')
        if tokens:
            nicki = []
                ...
                if token[0] in QUOTES:
                    for j, token2 in enumerate(tokens[i:]):
                        if token2[-1] in QUOTES:
                            ...
                            break
            if nicki:
                self[u'nickname'] = self._clean(u' '.join(tokens[nicki[0]:nicki[-1]+1]).strip(''.join(QUOTES)),
                                                     capitalize='name')
                tokens[nicki[0]:nicki[-1]+1] = []
        if tokens:
            self[u'middlename'] = self._clean(u' '.join(tokens), capitalize='name')
        namelist = []
        for attr in [u'title', u'firstname', u'middlename', u'nickname', u'prefix', u'lastname', u'suffix']:
            if attr in self:
                namelist.append(u'"%s"' % self[attr] if attr == u'nickname' else self[attr])
        self[u'name'] = u' '.join(namelist)
